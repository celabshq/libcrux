...
// The three unpacks are pure 32-bit-lane permutations within each 128-bit half
// (value-preserving rearrangements of whole i32 lanes), stated on the signed
// i32-lane view `lane32`.  Trusted axioms — each validated by its core-models
// `_mm256_unpack{lo,hi}_epi{32,64}` differential test + a lane transcription
// test in interpretations.rs.

// unpackhi_epi64 on i32 lanes: [lhs2,lhs3,rhs2,rhs3, lhs6,lhs7,rhs6,rhs7].
#[hax_lib::ensures(|result| fstar!(r#"forall (j: nat). j < 8 ==>
