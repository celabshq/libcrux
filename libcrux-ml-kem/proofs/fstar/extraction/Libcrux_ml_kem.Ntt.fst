...
let ntt_step_post
  ...
  (forall (i: nat).
      i < 16 ==>
      Libcrux_ml_kem.Vector.Traits.Spec.mont_i16_to_spec_fe (Seq.index (Libcrux_ml_kem.Vector.Traits.f_to_i16_array
                x)
            i) ==
...
let ntt_layer_int_vec_step
      ...
      (e_initial_coefficient_bound: usize)
     =
  let e_a_in:v_Vector = a in
  let e_b_in:v_Vector = b in
  let t:v_Vector =
  ...
  in
  let _:Prims.unit =
    let a_arr = Libcrux_ml_kem.Vector.Traits.f_to_i16_array e_a_in in
    let b_arr = Libcrux_ml_kem.Vector.Traits.f_to_i16_array e_b_in in
    ...
    let aux0 (i: nat)
      ...
      if i < 16
      then
        (Hacspec_ml_kem.Commute.Chunk.lemma_montgomery_multiply_lane_post_to_mod_q_eq (Seq.index b_arr
                i)
            zeta_r
            (Seq.index t_arr i);
          Hacspec_ml_kem.ModQ.lemma_mod_q_eq_unfold (v (Seq.index t_arr i))
            (v (Seq.index b_arr i) * v zeta_r * 169);
          Hacspec_ml_kem.Commute.Chunk.lemma_mont_mul_fe_commute_mont_mont zeta_r
            (Seq.index b_arr i)
            (Seq.index t_arr i);
          Hacspec_ml_kem.Commute.Chunk.lemma_add_fe_commute_mont (Seq.index a_arr i)
            (Seq.index t_arr i)
            (Seq.index x_arr i))
    in
      ...
      then
        (Hacspec_ml_kem.Commute.Chunk.lemma_montgomery_multiply_lane_post_to_mod_q_eq (Seq.index b_arr
          ...
          Hacspec_ml_kem.Commute.Chunk.lemma_mont_mul_fe_commute_mont_mont zeta_r
            (Seq.index b_arr i)
            (Seq.index t_arr i);
          Hacspec_ml_kem.Commute.Chunk.lemma_sub_fe_commute_mont (Seq.index a_arr i)
            (Seq.index t_arr i)
            (Seq.index y_arr i))
    in
  ...
  in
  let _:Prims.unit =
    reveal_opaque (`%ntt_step_post) (ntt_step_post #v_Vector e_a_in e_b_in a b zeta_r)
  in
...
let ntt_at_layer_4_plus
     ...
     =
  let e_zeta_i_init:usize = zeta_i in
  let e_re0:Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector = re in
  let e_re_init:t_Array v_Vector (mk_usize 16) = re.Libcrux_ml_kem.Vector.f_coefficients in
  let step:usize = mk_usize 1 <<! layer in
  let groups:usize = mk_usize 128 >>! layer in
  let step_vec_n:usize = step /! mk_usize 16 in
  let _:Prims.unit = lemma_layer_numeric_facts_fwd layer e_zeta_i_init in
  let _:Prims.unit =
    lemma_outer_inv_fwd_init #v_Vector
      e_re_init
      re.f_coefficients
      (v step_vec_n)
      (zs_of_fwd groups e_zeta_i_init)
      e_initial_coefficient_bound
      step
  in
  let (re: Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector), (zeta_i: usize) =
    Rust_primitives.Hax.Folds.fold_range (mk_usize 0)
      ...
      (fun temp_0_ round ->
          ...
          let _:Prims.unit =
            lemma_inner_inv_fwd_init #v_Vector e_re_init re.f_coefficients (v step_vec_n)
              (zs_of_fwd groups e_zeta_i_init) e_initial_coefficient_bound round step offset_vec
              step_vec
          in
          let re:Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector =
            Rust_primitives.Hax.Folds.fold_range offset_vec
              ...
              (fun re j ->
                  let re:Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector = re in
                  let j:usize = j in
                  let e_re_body_in:Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector = re in
                  let _:Prims.unit =
                    lemma_inner_inv_fwd_lookup #v_Vector e_re_init re.f_coefficients (v step_vec_n)
                      (zs_of_fwd groups e_zeta_i_init) e_initial_coefficient_bound offset_vec
                      step_vec j j;
                    lemma_inner_inv_fwd_lookup #v_Vector e_re_init re.f_coefficients (v step_vec_n)
                      (zs_of_fwd groups e_zeta_i_init) e_initial_coefficient_bound offset_vec
                      step_vec j (j +! step_vec)
                  in
                  ...
                  in
                  let _:Prims.unit =
                    lemma_offset_vec_fwd (v round) (v step) (v offset) (v offset_vec) (v step_vec_n);
                    FStar.Seq.Base.init_index_ (v groups)
                      (fun (r: nat{r < v groups}) ->
                          Libcrux_ml_kem.Vector.Traits.Spec.mont_i16_to_spec_fe (Libcrux_ml_kem.Polynomial.zeta
                                (e_zeta_i_init +! mk_usize 1 +! mk_usize r)))
                      (v round);
                    lemma_inner_step_maintains_fwd #v_Vector e_re_init e_re_body_in.f_coefficients
                      re.f_coefficients (v step_vec_n) (zs_of_fwd groups e_zeta_i_init)
                      e_initial_coefficient_bound offset_vec step_vec j (v round)
                      (Libcrux_ml_kem.Polynomial.zeta zeta_i) x y
                  in
                  re)
          in
          let _:Prims.unit =
            lemma_offset_vec_fwd (v round) (v step) (v offset) (v offset_vec) (v step_vec_n);
            lemma_inner_to_outer_fwd #v_Vector e_re_init re.f_coefficients (v step_vec_n)
              (zs_of_fwd groups e_zeta_i_init) e_initial_coefficient_bound round step offset_vec
              step_vec (round +! mk_usize 1) (offset_vec +! step_vec)
          in
          re, zeta_i <: (Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector & usize))
  in
  let _:Prims.unit =
    (let aux (round: nat)
          : Lemma
          (round < v groups ==>
            Seq.index (zs_of_fwd groups e_zeta_i_init) round ==
          ...
          (FStar.Seq.Base.init_index_ (v groups)
              (fun (r: nat{r < v groups}) ->
                  Libcrux_ml_kem.Vector.Traits.Spec.mont_i16_to_spec_fe (Libcrux_ml_kem.Polynomial.zeta
                        (e_zeta_i_init +! mk_usize 1 +! mk_usize r)))
              round;
            Hacspec_ml_kem.Commute.Bridges.lemma_zeta_eq_vzetas (e_zeta_i_init +! mk_usize 1 +!
                mk_usize round);
            assert (v (e_zeta_i_init +! mk_usize 1 +! mk_usize round) == v groups + round))
      in
      Classical.forall_intro aux)
  in
  let _:Prims.unit =
    lemma_offset_vec_fwd (v groups)
      (v step)
      (v groups * v step * 2)
      ((v groups * v step * 2) / 16)
      (v step_vec_n);
    (let auxb (i: nat)
          : Lemma
          (i < 16 ==>
            Libcrux_ml_kem.Polynomial.Spec.is_bounded_vector #v_Vector
              (e_initial_coefficient_bound +! mk_usize 3328)
              (re.f_coefficients.[ sz i ])) =
        if i < 16
        then
          lemma_outer_inv_fwd_lookup #v_Vector
            e_re_init
            re.f_coefficients
            (v step_vec_n)
            (zs_of_fwd groups e_zeta_i_init)
    ...
    lemma_postloop_cross_vec_fwd #v_Vector
      e_re_init
      re.f_coefficients
      (v step_vec_n)
      (zs_of_fwd groups e_zeta_i_init)
    ...
    FStar.Seq.Base.lemma_init_len (v groups)
      (fun (r: nat{r < v groups}) ->
          Libcrux_ml_kem.Vector.Traits.Spec.mont_i16_to_spec_fe (Libcrux_ml_kem.Polynomial.zeta (e_zeta_i_init +!
                  mk_usize 1 +!
                  mk_usize r)));
    lemma_groups_len_256 (v groups) (v step) (v step / 16);
    Hacspec_ml_kem.Commute.Ntt_bridge.lemma_layer_4_plus_to_poly_step #v_Vector
      e_re0
      re
      layer
      step
  ...
  let re:Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector =
    Rust_primitives.Hax.Folds.fold_range (mk_usize 0)
      ...
      (fun re j ->
          let re:Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector = re in
          let j:usize = j in
          let _:Prims.unit = Spec.Utils.impl_i16__abs_value (mk_i16 (- 1600)) in
          let t:v_Vector =
...
let ntt_vector_u
  ...
  in
  let re:Libcrux_ml_kem.Vector.t_PolynomialRingElement v_Vector =
    Libcrux_ml_kem.Polynomial.poly_barrett_reduce #v_Vector re
  in
  let _:Prims.unit =
    Hacspec_ml_kem.Commute.Chunk.lemma_poly_barrett_reduce_id (Hacspec_ml_kem.Commute.Chunk.to_spec_poly_plain
          #v_Vector
          re7)
  in
  re
